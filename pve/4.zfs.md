...
4. `zfs share` 共享
5. `zfs allow` 查看共享权限
6. `vim /etc/modprobe.d/zfs.conf` `options zfs_arc_max=34359738368` 最大内存占用 `1G：1*1024*1024*1024` 
   或者 `echo “options zfs zfs_arc_max = 42949672960 /etc/modprobe.d/zfs.conf` 
7. `zpool replace -f <pool> <old device> <new device>` 更换设备
8. `update-initramfs –u` 每次设置好都要更新下
## zfs去重
